...
import { askKnowledgeBase, type RagInput, type RagOutput } from '@/ai/flows/rag-flow';
...
export function TadeoChatbot() {
  ...
  const onSubmit = async (data: ChatFormValues) => {
    ...
    try {
      const input: RagInput = { query: data.message };
      const result: RagOutput = await askKnowledgeBase(input);
    ...
    <div className="flex flex-col h-full">
        <ScrollArea className="flex-1 p-4" ref={scrollAreaRef}>
            ...
            {messages.map((message, index) => (
                ...
                <div className={`rounded-lg p-3 text-sm max-w-sm ${message.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
